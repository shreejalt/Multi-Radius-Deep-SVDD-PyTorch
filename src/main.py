...
@click.option('--test_only', type=bool, default=False,
              help='Only test the model')
@click.option('--objective', type=click.Choice(['one-class', 'soft-boundary']), default='one-class',
              help='Specify Deep SVDD objective ("one-class" or "soft-boundary").')
@click.option('--nu', type=float, default=0.1, help='Deep SVDD hyperparameter nu (must be 0 < nu <= 1).')
@click.option('--device', type=str, default='cpu', help='Computation device to use ("mps", "cpu", "cuda", "cuda:2", etc.).')
...
@click.option('--normal_class', type=int, default=0, multiple=True,
              help='Specify the normal class of the dataset (all other classes are considered anomalous).')
@click.option('--use_multi_radius', type=bool, default=False,
              help='Specify the flag to use the multi-radius training')
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, objective, nu, device, seed,
         optimizer_name, lr, n_epochs, lr_milestone, batch_size, weight_decay, pretrain, ae_optimizer_name, ae_lr,
         ae_n_epochs, ae_lr_milestone, ae_batch_size, ae_weight_decay, n_jobs_dataloader, normal_class, test_only, use_multi_radius):
    """
    ...
    logger.info('Multi-radius training: %s' % str(use_multi_radius))
    ...
    deep_SVDD = DeepSVDD(cfg.settings['objective'], cfg.settings['nu'], cfg.settings['use_multi_radius'])
